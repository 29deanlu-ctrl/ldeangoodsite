...
class AdvancedClickerGame {
  constructor() {
    ...
    // Shuffle photos for collection
    this.shuffledPhotos = [...this.photos].sort(() => Math.random() - 0.5);

    // Set the first clickable image
    this.currentPhotoIndex = 0;
    document.getElementById('clickerImg').src = savedData.currentPhoto || this.photos[this.currentPhotoIndex];

    // Setup photo upgrades
    ...
    this.photoCount = this.photoUpgrades.filter(p=>p.purchased).length;
  ...
  submitScore = async(name) => {
    try{
      await db.collection('leaderboard').add({
        name, score: this.score, timestamp: firebase.firestore.FieldValue.serverTimestamp()
      });
